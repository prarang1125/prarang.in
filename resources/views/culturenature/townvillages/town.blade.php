...
<x-layout.pages.town :metaData="$metaData" :data="$town" :isAdsEnable="$isAdsEnable">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div class="lg:col-span-3  space-y-6 order-3 lg:order-1">
            ...
            <div class="bg-white rounded-2xl border border-green-200 shadow-sm overflow-hidden mb-6">
                ...
                <div class="px-6 pb-6 space-y-2" x-data="{ active: null }">
                    @php
                    $linksGroups = [];
                    $corpData = $town['extanded']['corporations'] ?? [];
        ...
        <div class="lg:col-span-6 space-y-6  order-1 lg:order-2">
            @if($town['ua_data'] != null)
            <div x-data="{ openUA: false }" class="mb-6">
                @php

                $uaArr = is_array($town['ua_data']) ? $town['ua_data'] : [];
                $uaCount = count($uaArr);
            ...
            @if($isAdsEnable)
            <div class="flex justify-center p-1 shadow bg-white rounded-lg">
                <img class="rounded" src="{{config('portal.sceh_ads.non-interaction')}}" alt="">
            </div>
            @endif
        </div>
