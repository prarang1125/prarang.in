<div>
    ...
    <div class="flex justify-center items-center">
        <button
            class="py-3 px-10 mx-20 rounded-pill border-2 border-green-500 bg-green-50 text-green-700 font-bold shadow-sm hover:bg-green-100 hover:border-green-600 hover:text-green-800 hover:shadow transition-all duration-200"
            data-bs-toggle="modal" data-bs-target="#joinUsModal">
            Join now
        </button>
    </div>
    ...
    <div class="modal fade" id="joinUsModal" tabindex="-1" aria-labelledby="joinUsModalLabel" aria-hidden="true"
        ...
        <div class="modal-dialog modal-dialog-centered rounded-xl">
            <div class="modal-content border-b rounded-xl">
                <div class="modal-header rounded-xl">
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body border-t mt-2">
                    {{-- @if ($showWelcome)
                    ...
                    <form wire:submit.prevent="register">
                        ...
                        <div class="mb-3 text-start">
                            <label class="form-label text-secondary fw-semibold">Preferred Social Media <span
                                    class="text-muted small">(optional)</span></label>
                            <div class="d-flex flex-wrap gap-3 mt-1">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" wire:model="preferredSocialMedia"
                                        value="WhatsApp" id="smWhatsApp">
                                    <label class="form-check-label" for="smWhatsApp">WhatsApp</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" wire:model="preferredSocialMedia"
                                        value="LinkedIn" id="smLinkedIn">
                                    <label class="form-check-label" for="smLinkedIn">LinkedIn</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" wire:model="preferredSocialMedia"
                                        value="Facebook" id="smFacebook">
                                    <label class="form-check-label" for="smFacebook">Facebook</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" wire:model="preferredSocialMedia"
                                        value="Instagram" id="smInstagram">
                                    <label class="form-check-label" for="smInstagram">Instagram</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" wire:model="preferredSocialMedia"
                                        value="X (Twitter)" id="smTwitter">
                                    <label class="form-check-label" for="smTwitter">X (Twitter)</label>
                                </div>
                            </div>
                            @error('preferredSocialMedia') <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div>
                        <p>Thank you for your interest. The social media posts for this Two-Country Bilateral
                            connections are not yet activated.</p>


                    </div>
                    <div class="d-flex justify-center items-center mt-3">
                        <a href="https://www.prarang.in/partners" target="_blank"
                            class="text-white p-2 shadow-none border-0 bg-primary rounded"
                            style="background-color: #2563eb !important;">Prarang Country Partnerships</a>
                    </div>
                </div>
            </div>
        </div>
