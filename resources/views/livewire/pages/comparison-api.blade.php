<div wire:init="loadServices">
    <div>
        ...
        <div class="action-buttons-corner" style="top: 92vh;height: 100px !important;">
            <button class=" action-button print" style="height: 42px !important;" onclick="printResponses()" title="Print" {{ $loading ? 'disabled' : '' }}>
                {{ $label['print'] ?? 'Print' }}
            </button>
            <form id="shareForm" action="{{ route('share.store') }}" method="POST" style="display: inline;">
                ...
                @endif
                <button type="button" onclick="handleShare()" class="action-button share" title="share" {{ $loading ? 'disabled' : '' }}>
        ...
        <div class="section-title">
            {{ $label['compare_share_report'] ?? 'Comparison Report' }}:{{ count($model) }}
            {{ $label['ai_modal_lable'] ?? 'A.I. Models' }}
        </div>

        <div class="info-box">
            <div class="timestamp">
                <strong>{{ $label['result_time_lable'] ?? 'Result Time (GMT)' }}:</strong> {{ now('UTC')->format('d-m-Y | H:i:s') }}
            </div>
        ...
        <div class="container-row">
            ...
            'perplexity' => 'https://framerusercontent.com/images/gcMkPKyj2RX8EOEja8A1GWvCb7E.jpg?width=2000&height=2000',
            ];
