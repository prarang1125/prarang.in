...
<script>
    // Enable parallel processing for AI comparison
    document.addEventListener('DOMContentLoaded', function() {
        // Find the AI comparison form
        const aiForm = document.querySelector('form[action*="ai.response"]');
        if (aiForm) {
            // Add parallel processing button
            ...
            if (submitButton) {
                ...
                parallelButton.className = 'btn btn-warning ms-2';
                parallelButton.textContent = 'Compare (Fast)';
                parallelButton.onclick = function() {
                    // Enable parallel processing for this form
                    handleParallelAIForm(aiForm);
                };
                submitButton.parentNode.appendChild(parallelButton);
            }
        }
    });
</script>
