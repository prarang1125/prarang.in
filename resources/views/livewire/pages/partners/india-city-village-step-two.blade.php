<x-partners.city-village-frame :step="$step">
    ...
    @if (count($cityData) > 0 && $step == 2)
                ...
                <table class="compact-table w-full text-sm text-left text-gray-500 border-collapse">
                    <thead class="text-xs text-gray-700  bg-gray-50 border-b">
                        <tr>
                            ...
                            <th class="px-1 py-0.5 text-xs border text-center">
                                Top 3 Languages &nbsp;<span class="text-blue-600"><a
                                        href="#language-star">*</a></span><br>
                        ...
                        @foreach ($cityData as $distId => $group)
                            ...
                            @if ($hasDhq)
                                <tr class="bg-white border-b hover:bg-gray-50 odd:bg-gray-50 text-gray-900">
                                    <td class="px-1 py-0.5 text-sm border text-center font-bold text-black">
                                        {{ $sr }}</td>
                                    <td class="px-1 py-0.5 text-sm border font-bold text-black">
                                        {{ $dhq['city_name'] ?? '' }}
                                    ...
                                    <td class="px-1 py-0.5 text-sm border font-medium text-black">
                                        {{ round((int) str_replace(',', '', $dhq['demo']['MSTR3'] ?? 0) / 1000) }}
                                    </td>
                                    <td class="px-1 py-0.5 text-sm border font-medium text-black">
                                        {{ round((int) str_replace(',', '', $dhq['internet']['city_population'] ?? 0) / 1000) }}
                                    </td>
                                    ...
                                    <td class="px-1 py-0.5 text-sm border text-center font-medium text-black">
                                        {{ isset($dhq['internet']['facebook_users'])
                                            ? round((float) ($dhq['internet']['facebook_users'] ?? 0)) . '%'
                                            : '-' }}
                                    </td>
                                    <td class="px-1 py-0.5 text-sm border text-center font-medium text-black">
                                        {{ isset($dhq['internet']['instagram_users'])
                                            ? round((float) ($dhq['internet']['instagram_users'] ?? 0)) . '%'
                                            : '-' }}
                                    </td>
                                    <td class="px-1 py-0.5 text-sm border text-center font-medium text-black">
                                        {{ isset($dhq['internet']['linkedin_users'])
                                            ? round((float) ($dhq['internet']['linkedin_users'] ?? 0)) . '%'
                                            : '-' }}
                                    </td>
                                    <td class="px-1 py-0.5 text-sm border text-center font-medium text-black">
                                        {{ isset($dhq['internet']['twitter_users'])
                                            ? round((float) ($dhq['internet']['twitter_users'] ?? 0)) . '%'
                                            : '-' }}
                                    </td>
                                    ...
                                    <td rowspan="{{ $rowspan }}"
                                        class="px-1 py-0.5 text-sm border min-w-[250px] align-middle">
                                        @if (isset($dhq['languages']) && is_array($dhq['languages']))
                                            ...
                                            <div class="flex flex-col gap-2 justify-center h-full">
                                                @foreach ($topLangs as $langCode => $langData)
                                                    @php
                                                        $langName = isset($lang_titles[$langCode])
                                                            ? $lang_titles[$langCode]
                                                            : $langCode;
                                                    @endphp
                                                    <label class="flex items-center gap-2 cursor-pointer w-fit">
                                                        <input type="checkbox"
                                                            wire:key="dist-{{ $distId }}-{{ $langCode }}"
                                                        ...
                                                        <span
                                                            class="text-sm text-gray-700 font-medium">{{ $langName }}
                                                            <span
                                                                class="text-xs text-gray-500 font-normal">({{ number_format($langData['percentage'], 1) }}%)</span></span>
                                                    </label>
                                                @endforeach
                                            </div>
                                        @endif
                            ...
                            @foreach ($towns as $town)
                                <tr class="bg-white border-b hover:bg-gray-50 odd:bg-gray-50 text-gray-900">
                                    <td class="px-1 py-0.5 text-sm border text-center font-bold text-black">
                                        {{ $sr }}.{{ chr($subCharCode++) }}</td>
                                    <td class="px-1 py-0.5 text-sm border font-normal text-black">
                                        {{ $town['city_name'] ?? '' }}
                                    ...
                                    @endif
                                    <td class="px-1 py-0.5 text-sm border font-medium text-black">
                                        {{ round((int) str_replace(',', '', $town['demo']['MSTR3'] ?? 0) / 1000) }}
                                    </td>
                                    <td class="px-1 py-0.5 text-sm border font-medium text-black">
                                        {{ round((int) str_replace(',', '', $town['internet']['city_population'] ?? 0) / 1000) }}
                                    </td>
                            ...
                            @foreach ($villages as $village)
                                <tr class="bg-white border-b hover:bg-gray-50 odd:bg-gray-50 text-gray-900">
                                    ...
                                    @endif
                                    <td class="px-1 py-0.5 text-sm border font-medium text-black">
                                        {{ round((int) str_replace(',', '', $village['demo']['MSTR3'] ?? 0) / 1000) }}
                                    </td>
                                    <td class="px-1 py-0.5 text-sm border font-medium text-black">
                                        {{ round((int) str_replace(',', '', $village['internet']['city_population'] ?? 0) / 1000) }}
                                    </td>
                                    <td class="px-1 py-0.5 text-sm border text-center font-medium text-black">-</td>
                                    <td class="px-1 py-0.5 text-sm border text-center font-medium text-black">
        ...
        <div class="p-1 border border-gray-200 rounded-md text-justify">
            ...
            <ol class="list-decimal pl-5 space-y-1 text-xs">
                <li>Internet Users data for cities and villages is available but all other metrics (Literacy, Social
                    Media
                    %, Cyber Risk Index) are District Capital level only.</li>
                <li>Language profile is based on the District Capital's recorded data. Only Mother Tongue is considered
                    for
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body text-center pt-2 pb-4">
                            <p class="mb-0 text-gray-800 text-lg">We recommend selecting at least 5 Knowledge Web
                                Geographies for each language, for optimal budgeting.</p>
                        </div>
                        <div class="modal-footer border-0 justify-content-center gap-3">
                            <button type="button"
                                class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg font-medium transition-colors"
                                data-bs-dismiss="modal">Revise plan</button>
                            <button type="button" x-on:click="window.scrollTo({ top: 30, behavior: 'smooth' })"
                                wire:click="confirmLanguageSelection" data-bs-dismiss="modal"
                                wire:loading.attr="disabled" wire:target="confirmLanguageSelection"
    ...
    @if (count($cityData) > 0 && $step == 3)
        ...
        <section class="mb-8" x-data="{ showPlanDetails: false, showProcessingModal: false }">
            ...
            <div class="p-2 overflow-x-auto">
                <table
                    class="compact-table w-full text-sm text-left text-gray-700 border-collapse border border-gray-300"
                    x-data="{ showSocial: false }" :class="showSocial ? 'show-social' : ''">
                    <thead class="text-xs bg-gray-50 border-b border-gray-300">
                        <tr>
                            <th rowspan="2"
                                class="px-1 py-0.5 border text-center align-middle font-bold text-gray-800">S.
                                No.
                            </th>
                            <th rowspan="2"
                                class="px-1 py-0.5 border text-center align-middle font-bold text-gray-800">City
                            </th>
                            <th rowspan="2"
                                class="px-1 py-0.5 border text-center align-middle font-bold text-gray-800">
                                State
                            </th>
                            <th rowspan="2"
                                class="px-1 py-0.5 border text-center align-middle font-bold text-gray-800">
                            ...
                            <th colspan="3"
                                class="px-1 py-0.5 border text-center font-bold text-gray-800 bg-indigo-100">
                        ...
                        <tr>
                            ...
                            @foreach ($pricing['base'] as $key => $value)
                                <td class="text-center text-indigo-700 font-semibold text-xs">₹{{ $value['monthly'] }}
                                </td>
                            @endforeach
...
$internetUsersInThousands = (float) ($loc['internet']['internet_users'] ?? 0);
                                        ...
                                        $totalInternetUsersper =
                                            $totalPop2026 > 0 ? ($totalInternetUsers / $totalPop2026) * 100 : 0;
                                        ...
                                        <tr class="bg-white border-b hover:bg-gray-50 odd:bg-gray-50">
                                            @if ($firstLang)
                                                <td rowspan="{{ $langCount }}"
                                                    ...
                                                    {{ $srNo }}@if ($locIdx > 0 || !$hasDhq)
                                                        .{{ chr($subAlphaCharCode++) }}
                                                    @endif
                                                </td>

                                                <td rowspan="{{ $langCount }}"
                                                    class="px-1 py-0.5 text-sm border font-semibold text-gray-900 align-middle">
                                                    {{ $loc['city_name'] ?? '' }} <br><span
                                                        class="text-xs font-normal">({{ $loc['location_type'] === 'City'
                                                            ? 'City'
                                                            : ($loc['location_type'] === 'District Capital'
                                                                ? 'District Capital'
                                                                : 'Village') }})</span>
                                                </td>
                                                ...
                                                @if ($isFirstLocation)
                                                    <td rowspan="{{ $totalRows }}"
                                                        class="px-1 py-0.5 text-sm border align-middle text-center">
                                                        {{ $dhq['demo']['MSTR1'] ?? '-' }}
                                                    </td>
                                                @endif

                                                <td rowspan="{{ $langCount }}"
                                                    class="px-1 py-0.5 text-sm border text-center align-middle">
                                            ...
                                            @php
                                                $isFreeDistrictCapital =
                                                    $loc['location_type'] === 'District Capital' &&
                                                    (count($towns) > 0 || count($villages) > 0);
                                            @endphp
                                            <td
                                                class="px-1 py-0.5 text-sm border text-center align-middle bg-indigo-50/30">
                                                <div class="flex flex-col items-center justify-center">
                                                    <input type="radio"
                                                        name="plan_{{ $loc['city_id'] }}_{{ $langCode }}"
                                                    ...
                                                    @if ($isFreeDistrictCapital)
                                                        <span
                                                            class="text-[10px] text-green-600 font-bold mt-0.5">Free</span>
                                                    @endif
                                                </div>
                                            </td>
                                            <td
                    ...
                    <tfoot class="bg-gray-100 border-t-2 border-gray-400 font-bold text-gray-800">
                        <tr>
                            ...
                            <td class="px-1 py-0.5 border text-center text-sm">{{ number_format($totalPop2026, 0) }}
                            </td>
                            <td class="px-1 py-0.5 border text-center text-sm"></td>
                            <td class="px-1 py-0.5 border text-center text-sm">
                                {{ number_format($totalInternetUsers, 0) }}
                            </td>
                ...
                @section('p-footer')
                    <div class="flex justify-between">
                        <button wire:click="changeStep('back')" wire:loading.attr="disabled" wire:target="changeStep"
                            ...
                            <span wire:loading.remove wire:target="changeStep"><i class="bi bi-arrow-left mr-2"></i>
                                Back</span>
                            <span wire:loading wire:target="changeStep" class="flex items-center gap-2">
    ...
    @if (count($cityData) > 0 && $step == 4)
        ...
        <section class="mb-8">
            <div class="p-2 overflow-x-auto">
                <table
                    ...
                    <thead class="text-xs bg-gray-50 border-b border-gray-300">
                        <tr>
                            ...
                            <th colspan="6"
                                class="px-1 py-0.5 border text-center font-bold text-gray-800 bg-green-100">
                                Standard Solutions</th>
                            <th colspan="9"
                                class="px-1 py-0.5 border text-center font-bold text-gray-800 bg-blue-100">KW
                                (City)-Interaction Solution*</th>
                        </tr>
                        <tr>
                            <th rowspan="2" colspan="6"
                                class="px-1 py-0.5 border text-center font-bold text-xs text-gray-800 bg-green-50">
                                <b>Content:</b> (Automated Text Header) <br>
                        ...
                        <tr>
                            <th class="px-1 py-0.5 border text-center font-bold text-xs text-gray-800 bg-blue-50">
                                4 Posts<br><span
                                    class="font-normal">(₹{{ number_format($pricing['optional_solutions']['Weekly Posts']['monthly']) }}/month)</span>
                            </th>
                            <th class="px-1 py-0.5 border text-center font-bold text-xs text-gray-800 bg-blue-50">15
                                Posts<br><span
                                    class="font-normal">(₹{{ number_format($pricing['optional_solutions']['Alternate Day Posts']['monthly']) }}/month)</span>
                            </th>
                            <th class="px-1 py-0.5 border text-center font-bold text-xs text-gray-800 bg-blue-50">
                                31 Posts<br><span
                                    class="font-normal">(₹{{ number_format($pricing['optional_solutions']['Daily Posts']['monthly']) }}/month)</span>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        @foreach ($cityData as $distId => $group)
                                ...
                                @foreach ($validLocations4 as $locIdx4 => $item4)
                                    ...
                                    @foreach ($langsWithPlan as $lc4 => $ld4)
                                        @php
                                            ...
                                            $isFreeDistrictCapital4 =
                                                $loc4['location_type'] === 'District Capital' && $hasSubLocations4;
                                            if ($selHosting) {
                                                if (str_ends_with((string) $selHosting, '-prarang')) {
                                                    $hostingType4 = 'prarang';
                                                    $hostingLabel4 = 'Prarang.in';
                                                    $enrollCost4 = $isFreeDistrictCapital4
                                                        ? 0
                                                        : $pricing['base']['Prarang.in']['monthly'];
                                        ...
                                        <tr class="bg-white border-b hover:bg-gray-50 odd:bg-gray-50">
                                            ...
                                            @if ($firstLang4)
                                                <td rowspan="{{ $langCount4 }}"
                                                    ...
                                                    {{ $loc4['city_name'] ?? '' }}
                                                    <br><span
                                                        class="text-xs font-normal text-gray-500">({{ $loc4['location_type'] === 'City'
                                                            ? 'City'
                                                            : ($loc4['location_type'] === 'District Capital'
                                                                ? 'District Capital'
                                                                : 'Village') }})</span>
                                                </td>
                                                @if ($isFirstLoc4)
                                                    <td rowspan="{{ $totalRows4 }}"
                                                        class="px-1 py-0.5 text-sm border align-middle text-center">
                                                        {{ $dhq['demo']['MSTR1'] ?? '-' }}
                                                    </td>
                                                @endif
                                            @endif
                                            <td
                                                class="px-1 py-0.5 text-xs border text-center align-middle font-medium">
                                                {{ $langName4 }}
                                            </td>
                            ...
                            @if ($hostingType4) <span class="text-green-600">✓</span> @else <span
                                class="text-gray-300">-</span> @endif
                                            ...
                                            {{-- Enrollment --}}
                                            <td colspan="6"
                                                class="px-1 py-0.5 text-xs border text-center align-middle bg-green-50/40">
                                                @if ($hostingType4)
                                                    @if ($enrollCost4 == 0)
                                                        <span class="text-green-700 font-semibold">✓ ₹0</span>
                                                    @else
                                                        <span class="text-green-700 font-semibold">✓
                                                            ₹{{ number_format($enrollCost4) }}</span>
                                                    @endif
                                                @else
                        ...
                        {{-- District Analytics --}}
                            ...
                            @if ($hostingType4) <span class="text-green-600">✓</span> @else <span
                                class="text-gray-300">-</span> @endif
                                            ...
                                            {{-- City Posts: Weekly --}}
                                            <td
                                                class="px-1 py-0.5 text-xs border text-center align-middle bg-blue-50/30">
                                                <input type="radio"
                                                    name="citypost_{{ $loc4['city_id'] }}_{{ $lc4 }}"
                                                    wire:model.live="selectedCityPosts.{{ $planKey4 }}"
                                                    value="{{ $planKey4 }}-weekly"
                                ...
                                class="text-gray-300">-</span></td> --}}
                                            ...
                                            <td
                                                class="px-1 py-0.5 text-xs border text-center align-middle bg-blue-50/30">
                                                <input type="checkbox"
                                                    wire:model.live="selectedYellowPages.{{ $planKey4 }}"
                                                    value="50000"
                                            ...
                                            <td
                                                class="px-1 py-0.5 text-xs border text-center align-middle bg-blue-50/30">
                                                <input type="checkbox"
                                                    wire:model.live="selectedOutdoorAds.{{ $planKey4 }}"
                                                    value="50000"
                                                    class="w-4 h-4 text-blue-600 rounded cursor-pointer">
                                            </td>
                                            {{-- District Analytics --}}
                                            <td
                                                ...
                                                <input disabled type="checkbox"
                                                    wire:model="selectedDistrictAnalytics.{{ $planKey4 }}"
                                                    value="1"
                                                    class="w-4 h-4 text-blue-600 rounded cursor-pointer">
                                            </td>
                                            {{-- Semiotics --}}
                                            <td
                                                class="px-1 py-0.5 text-xs border text-center align-middle bg-blue-50/30">
                                                <input disabled type="checkbox"
                                                    wire:model.live="selectedSemiotics.{{ $planKey4 }}"
                                                    value="1"
                                                    class="w-4 h-4 text-blue-600 rounded cursor-pointer">
                                            </td>
                                            {{-- Partner Metrics --}}
                                            <td
                                                class="px-1 py-0.5 text-xs border text-center align-middle bg-blue-50/30">
                                                <input disabled type="checkbox"
                                                    wire:model.live="selectedPartnerMetrics.{{ $planKey4 }}"
                                                    value="1"
                                                    class="w-4 h-4 text-blue-600 rounded cursor-pointer">
                                            </td>
                                        </tr>
                    ...
                    @php
                        $weeklyTotalCalc =
                            collect($selectedCityPosts)->filter(fn($val) => str_ends_with($val, '-weekly'))->count() *
                            $pricing['optional_solutions']['Weekly Posts']['monthly'];
                        $alternateDayTotalCalc =
                            collect($selectedCityPosts)
                                ->filter(fn($val) => str_ends_with($val, '-alternateday'))
                                ->count() * $pricing['optional_solutions']['Alternate Day Posts']['monthly'];
                        $dailyTotalCalc =
                            collect($selectedCityPosts)->filter(fn($val) => str_ends_with($val, '-daily'))->count() *
                            $pricing['optional_solutions']['Daily Posts']['monthly'];
                        $yellowPagesTotalCalc =
                            collect($selectedYellowPages)->filter()->count() *
                            $pricing['optional_solutions']['City Yellow Pages (DM)']['monthly'];
                        $outdoorAdTotalCalc =
                            collect($selectedOutdoorAds)->filter()->count() *
                            $pricing['optional_solutions']['City Outdoor Ad Analytics']['monthly'];
                                    ...
                                    if (!empty($selectedLanguages["{$distId}-{$langCode}"])) {
                                        ...
                                        foreach ($tempLocations as $tLoc) {
                                            ...
                                            if (!empty($selectedPlans[$planKey])) {
                                                ...
                                                $isFree =
                                                    $tLoc['location_type'] === 'District Capital' && $hasSubLocations;

                                                if (str_ends_with((string) $selHosting, '-prarang')) {
                                                    $totalHostingCost += $isFree
                                                        ? 0
                                                        : $pricing['base']['Prarang.in']['monthly'];
                                                } elseif (
                                                    str_ends_with((string) $selHosting, '-yourwebsite') ||
                                                    str_ends_with((string) $selHosting, '-newwebsite')
                                                ) {
                    ...
                    <tfoot class="bg-gray-100 border-t-2 border-gray-400 font-bold text-gray-800">
                        <tr>
                            ...
                            <td colspan="2" class="px-1 py-0.5 border text-left text-sm">{{ $totalRowsCount }}
                            </td>
                ...
                <p class="text-xs text-gray-500 mt-2">*Only District-Capital pricing for City-Interaction Solutions
                    included above. Other Cities (Non-Capital) and Villages Interaction pricing are expected to be
                    similar.</p>
                    ...
                    <div class="flex justify-between">
                        ...
                        <div class="flex gap-4">
                            ...
                            <button wire:click="confirmCityPostsSelection" wire:loading.attr="disabled"
                                ...
                                <span wire:loading wire:target="confirmCityPostsSelection"
