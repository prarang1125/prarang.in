<div class="min-h-screen  py-2 px-4 sm:px-6 lg:px-8">
    ...
    <div class="rounded shadow-lg">
        ...
        <div class="p-3 md:p-12">
            {{-- ── Categorised cities section (town mode only) ─────────────── --}}
            @if ($type === 'town')
                        ...
                        <div class="relative">
                            <button type="button" @click="if({{ $cat_state ? 'true' : 'false' }}) open = !open"
                                ...
                                <span
                                    ...
                                    {{ $selected_sc ? collect($sc)->where('id', $selected_sc)->first()->name ?? 'Selected' : 'Choose...' }}
                                </span>
                                <svg class="w-4 h-4 text-slate-400 group-hover:text-blue-500 transition-transform duration-300"
                                    :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                            ...
                            <span>{{ count($sc) ? 'Total : ' . count($sc) : '' }}</span>
                            <div x-show="open" @click.away="open = false" x-cloak x-transition
                                class="absolute z-50 w-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-2xl p-3 overflow-hidden">
                                <div class="relative mb-2">
                                    <input type="text" x-model="search" placeholder="Search..."
                        ...
                        <div class="relative">
                            <button type="button" @click="if({{ $cat_state ? 'true' : 'false' }}) open = !open"
                                ...
                                <span
                                    ...
                                    {{ $selected_dhq ? collect($dhq)->where('id', $selected_dhq)->first()->name ?? 'Selected' : 'Choose...' }}
                                </span>
                                <svg class="w-4 h-4 text-slate-400 group-hover:text-blue-500 transition-transform duration-300"
                                    :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                            ...
                            <span>{{ count($dhq) ? 'Total : ' . count($dhq) : '' }}</span>
                            <div x-show="open" @click.away="open = false" x-cloak x-transition
                                class="absolute z-50 w-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-2xl p-3 overflow-hidden">
                                <div class="relative mb-2">
                                    <input type="text" x-model="search" placeholder="Search..."
                        ...
                        <div class="relative">
                            <button type="button" @click="if({{ $cat_state ? 'true' : 'false' }}) open = !open"
                                ...
                                <span
                                    ...
                                    {{ $selected_ua ? collect($ua)->where('id', $selected_ua)->first()->name ?? 'Selected' : 'Choose...' }}
                                </span>
                                <svg class="w-4 h-4 text-slate-400 group-hover:text-blue-500 transition-transform duration-300"
                                    :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                            ...
                            <span>{{ count($ua) ? 'Total : ' . count($ua) : '' }}</span>
                            <div x-show="open" @click.away="open = false" x-cloak x-transition
                                class="absolute z-50 w-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-2xl p-3 overflow-hidden">
                                <div class="relative mb-2">
                                    <input type="text" x-model="search" placeholder="Search..."
                        ...
                        <div class="relative">
                            <button type="button" @click="if({{ $cat_state ? 'true' : 'false' }}) open = !open"
                                ...
                                <span
                                    ...
                                    {{ $selected_mcp ? collect($mcp)->where('id', $selected_mcp)->first()->name ?? 'Selected' : 'Choose...' }}
                                </span>
                                <svg class="w-4 h-4 text-slate-400 group-hover:text-blue-500 transition-transform duration-300"
                                    :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                            ...
                            <span>{{ count($mcp) ? 'Total : ' . count($mcp) : '' }}</span>
                            <div x-show="open" @click.away="open = false" x-cloak x-transition
                                class="absolute z-50 w-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-2xl p-3 overflow-hidden">
                                <div class="relative mb-2">
                                    <input type="text" x-model="search" placeholder="Search..."
                        ...
                        <div class="relative">
                            <button type="button" @click="if({{ $cat_state ? 'true' : 'false' }}) open = !open"
                                ...
                                <span
                                    ...
                                    {{ $selected_smc ? collect($smc)->where('id', $selected_smc)->first()->name ?? 'Selected' : 'Choose...' }}
                                </span>
                                <svg class="w-4 h-4 text-slate-400 group-hover:text-blue-500 transition-transform duration-300"
                                    :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                            ...
                            <span>{{ count($smc) ? 'Total : ' . count($smc) : '' }}</span>
                            <div x-show="open" @click.away="open = false" x-cloak x-transition
                                class="absolute z-50 w-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-2xl p-3 overflow-hidden">
                                <div class="relative mb-2">
                                    <input type="text" x-model="search" placeholder="Search..."
                            ...
                            <h2 class="text-xl md:text-xl text-slate-800 tracking-tighter mb-2">
                                ...
                                <span class="text-gray-500">(with Population 1 Lakh+)
                                    <a href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#box25l"
                                        class="text-blue-600 hover:text-blue-800"> Click Here</a>
                                </span>
                            </h2>
                        </div>
                    </div>
                    <div class="col-span-2 flex flex-col justify-center items-center border-2 p-2 border-gray-300 rounded-lg shadow">
            ...
            <div class="">
                <div
                    ...
                    <div class="space-y-3" x-data="{ open: false, search: '' }">
                        <div class="flex items-center gap-3 mb-2">
                            ...
                            <label class="text-sm font-bold text-slate-500 uppercase tracking-widest">
                                Select District {{ count($districts) !== 0 ? '#' . count($districts) : ' ' }}
                    ...
                    {{-- ── Step 3 (village only): Sub-District ────────────────────── --}}
                    @if ($type === 'village')
                    <div class="space-y-3" x-data="{ open: false, search: '' }">
                        <div class="flex items-center gap-3 mb-2">
                            <span
                            ...
                            <label class="text-sm font-bold text-slate-500 uppercase tracking-widest">
                                Select Block {{ count($subDistricts) !== 0 ? '#' . count($subDistricts) : ' ' }}
                            </label>
                        </div>
                    ...
                    <div class="space-y-3" x-data="{ open: false, search: '' }">
                        <div class="flex items-center gap-3 mb-2">
                            ...
                            @endphp
                            <span
                                class="flex items-center justify-center w-8 h-8 rounded-xl {{ $stepDone ? 'bg-green-100 text-green-600' : ($stepActive ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-300') }} font-bold text-sm transition-colors duration-300">
                                <div wire:loading wire:target="{{ $type === 'town' ? 'state' : 'subDistrict' }}">
                                    <svg class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10"
                                            stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor"
                            ...
                            <label class="text-sm font-bold text-slate-500 uppercase tracking-widest">
                                Select {{ ucfirst($type == 'town' ? 'City' : 'Village') }}
                                {{ count($type === 'town' ? $towns : $villages) !== 0 ? '#' . count($type === 'town' ? $towns : $villages) : ' ' }}
                            </label>
                        </div>
                ...
                </div>{{-- end main filter grid --}}

                {{-- ── Confirm button (normal village/town flow) ───────────────── --}}
                @if (
                ...
                '/' .
                url_encoder($state . '-' . $urlDistrict . '-' . ($type === 'town' ? $urlTown : $village)) .
                '/' .
                $this->selectedSlug;
                @endphp
                ...
                [
                'type' => 'h_r_webs',
                'lable' => 'Hindi Webs: Ready for Interactions',
                'data' => [
                ...
                [
                'type' => 'm_r_webs',
                'lable' => 'Marathi Webs: Ready for Interactions',
                'data' => [['title' => 'Pune', 'slug' => 'pune']],
                ],
                ];
                @endphp
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-3">
                            ...
                            [
                            'label' => $city['title'],
                            'url' => "https://prarang.in/{$city['slug']}",
                            ],
                            ];
                ...
                <style>
                    ...
                    .md\:p-12 .hover\:shadow-md .overflow-hidden {
                        display: flex;
                ...
                <div class="grid grid-cols-1 gap-3">
                    <div
                        class="group flex items-center bg-white border border-slate-200 rounded-xl overflow-hidden hover:shadow-md transition duration-300 max-w-md">
                        ...
                        <div class="flex-1 p-3">
                            <h4 class="text-sm font-bold text-slate-900 leading-tight">Ram Nagar</h4>
                            <p class="text-xs text-slate-500 mt-1">Bareilly, Uttar Pradesh</p>
                            <a href="https://prarang.in/village/OSAtMTMwLTEyOTgyMw/ram-nagar" target="_blank"
                                class="inline-flex items-center gap-1 text-xs font-semibold text-blue-600 hover:text-blue-700 mt-2">
                                View
