            ...
            @if ($type === 'village')
                ...
                @else
                    <div class="pb-4 text-center">
                        <h2 class="text-xl md:text-xl  text-slate-800 tracking-tighter mb-2">
                            Explore All <span
                                class="text-blue-700 font-extrabold text-[20px] md:text-[22px]">6,331</span>
        ...
        <div class="">
            ...
            <div
                    ...
                    <div class="relative">
                        ...
                        <div x-show="open" @click.away="open = false" x-cloak
                            ...
                            <div class="relative mb-3">
                                <input type="text" x-model="search" placeholder="Search states..." autocomplete="off"
                ...
                <div class="space-y-3" x-data="{ open: false, search: '' }">
                    ...
                    <div class="flex items-center gap-3 mb-2">
                        ...
                        <label class="text-sm font-bold text-slate-500 uppercase tracking-widest">Select
                            {{ ucfirst($type == 'town' ? 'City' : 'Villages') }}
                            {{ count($type === 'town' ? $towns : $villages) !== 0
                    ...
                    <div class="relative">
                        ...
                        <button type="button" @click="if({{ $isStepActive ? 'true' : 'false' }}) open = !open"
                            ...
                            <span
                                ...
                                @if ($type === 'town')
                                    {{ $town
                                        ? collect($towns)->where('id', $town)->first()->name ??
                                            (collect($towns)->where('id', $town)->first()['name'] ?? 'Selected')
                                        : 'Choose
                                                                            a
                                                                            town...' }}
                                @else
                                    {{ $village
                                        ? collect($villages)->where('id', $village)->first()->name ??
                        ...
                        <div x-show="open" @click.away="open = false" x-cloak
                            ...
                            <div class="max-h-60 overflow-y-auto custom-scrollbar-premium">
                                ...
                                @empty
                                    <div class="px-4 py-3 text-sm text-slate-400 font-medium">Select a
                                        {{ $type === 'town' ? 'district' : 'tehsil' }}
                                        first
                                    </div>
                                @endforelse
                            </div>
                ...
                <div class="mt-12 pt-8 border-t border-slate-100">
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-6">
                        ...
                        <div class="w-full sm:w-auto transition-all duration-500 opacity-100 scale-100">
                            <a target="_blank"
                                href="{{ url('/') }}/{{ $type === 'town' ? 'city' : 'village' }}/{{ url_encoder($state . '-' . $district . '-' . ($type === 'town' ? $town : $village)) }}/{{ $this->selectedSlug }}"
                ...
                <div class="mt-8 pt-8 border-t border-slate-100">
                    ...
                    @if ($selected_sc || $selected_dhq || $selected_ua || $selected_mcp || $selected_smc)
                        <div class="mt-8 flex justify-end">
                            <div class="w-full sm:w-auto transition-all duration-500 opacity-100 scale-100">
                                <a target="_blank"
                                    href="{{ url('/') }}/city/{{ url_encoder($state . '-' . $district . '-' . $town) }}/{{ $this->selectedSlug }}"
                                    class="inline-flex items-center justify-center w-full sm:w-auto px-10 py-4
                                    bg-blue-600
                                    text-white font-black text-sm tracking-[0.1em] uppercase rounded-2xl shadow-xl
                                    shadow-blue-200 hover:bg-blue-700 hover:shadow-blue-300 hover:-translate-y-1
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-3">
                                @foreach ($group['data'] as $city)
                                    @php
                                        $links = $city['links'] ?? [
                                            ['label' => $city['title'], 'url' => "https://prarang.in/{$city['slug']}"],
                                        ];
            ...
            @else
                ...
                <div class="grid grid-cols-1 gap-3">
                    ...
                    <!-- Small Compact Card -->
                    <div
                        class="group flex items-center bg-white border border-slate-200
        rounded-xl overflow-hidden hover:shadow-md transition duration-300
        max-w-md">
                ...
                text-blue-600 hover:text-blue-700 mt-2">
                                View
