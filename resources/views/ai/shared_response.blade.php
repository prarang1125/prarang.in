...
<x-layout.main.base :metaData="$metaData">
    ...
    <div class="section-title">Comparison Report
        :{{ (isset($content) ? 1 : 0) + (isset($gptResponse) ? 1 : 0) + (isset($geminiResponse) ? 1 : 0) + (isset($deepSeekResponse) ? 1 : 0) + (isset($metaResponse) ? 1 : 0) + (isset($claudeResponse) ? 1 : 0) }}
        A.I. Models</div>
                ...
                <div class="model-links-wrapper">
                    <div class="model-links-row">
                        ...
                        @if (isset($umanResponse))
                            ...
                            <a class="model-link"
                                onclick="scrollToResponse('upmana-container')">({{ chr(96 + $modelCount) }})Upmana</a>
                        @endif

                        @if (isset($gptResponse))
                            @php $modelCount++; @endphp
                            <a class="model-link"
                                onclick="scrollToResponse('gpt-container')">({{ chr(96 + $modelCount) }})ChatGPT</a>
                        @endif

                        @if (isset($geminiResponse))
                            @php $modelCount++; @endphp
                            <a class="model-link"
                                onclick="scrollToResponse('gemini-container')">({{ chr(96 + $modelCount) }})Gemini</a>
                        @endif

                        @if (isset($claudeResponse))
                            @php $modelCount++; @endphp
                            <a class="model-link"
                                onclick="scrollToResponse('claude-container')">({{ chr(96 + $modelCount) }})Claude</a>
                        @endif

                        @if (isset($deepSeekResponse))
                            @php $modelCount++; @endphp
                            <a class="model-link"
                                onclick="scrollToResponse('deepseek-container')">({{ chr(96 + $modelCount) }})DeepSeek</a>
                        @endif

                        @if (isset($metaResponse))
                            @php $modelCount++; @endphp
                            <a class="model-link"
                                onclick="scrollToResponse('meta-container')">({{ chr(96 + $modelCount) }})Meta</a>
                        @endif
                    </div>
                </div>
            </div>
    ...
    <div class="container-row">
        ...
        @if (isset($gptResponse))
            <div class="response-container" id="gpt-container">
                <img src="https://upload.wikimedia.org/wikipedia/commons/0/04/ChatGPT_logo.svg" alt="GPT Logo"
                    class="ai-logo">
                <div class="response-content">
        ...
        @if (isset($geminiResponse))
            <div class="response-container" id="gemini-container">
                <img src="https://upload.wikimedia.org/wikipedia/commons/8/8a/Google_Gemini_logo.svg"
                    alt="Google Gemini Logo" class="ai-logo">
                <div class="response-content">
        ...
        @if (isset($claudeResponse))
            <div class="response-container" id="claude-container">
                <img src="https://upload.wikimedia.org/wikipedia/commons/7/78/Anthropic_logo.svg" alt="Anthropic Logo"
                    class="ai-logo">
                <div class="response-content">
        ...
        @if (isset($grokResponse))
            <div class="response-container" id="grok-container">
                <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/4/4d/X_Logo.svg/2048px-X_Logo.svg.png"
                    alt="Grok Logo" class="ai-logo">
                <div class="response-content">
