...
<x-layout.main.base :metaData="$metaData">
  <div class="flex flex-col items-center py-1">
    ...
    <a class="p-0 mt-1 text-blue-600 hover:text-blue-800 font-medium transition-colors text-sm" href="https://sceh.net/"
      target="_blank">https://sceh.net/</a>
  </div>
    ...
    <div class="row g-4">
      ...
      <div class="col-xl-12 col-lg-12 col-md-12">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden h-100 flex flex-col">
          <div class="bg-blue-600 text-white p-2 text-center font-bold text-sm tracking-wide uppercase">City/Village
            Webs - Interaction</div>
            ...
            <table class="table table-hover table-striped table-sm m-0 border-0 align-middle">
              ...
              <tbody>
                ...
                <tr class="transition-colors hover:bg-blue-50">
                  ...
                  <td class="text-xs">
                    <livewire:modules.partner.util.show-dhq-ranks :title="$item['district_name']"
                      :code="$item['dhq_code']" />
                  </td>
                  ...
                  <td class="text-xs text-right font-medium text-gray-700">{{number_format($cityPop[$item['dhq_code']]*
                    pow(1 + (str_replace('%','',$item['AEGR_pct']) / 100), 15)) ?? 0}}</td>
                  <td class="text-xs text-center">
                    <a target="_blank" href="https://www.prarang.in/{{strtolower($item['city'])}}"
                      class="inline-block bg-blue-100 text-blue-700 px-2 py-1 rounded text-[8px]  hover:bg-blue-200 transition-colors whitespace-nowrap">Click
                      <br> Here</a>
                  </td>
                </tr>
                @endforeach
            ...
            <table class="table table-hover table-striped table-sm m-0 border-0 align-middle">
              ...
              <tbody>
                ...
                <tr class="transition-colors hover:bg-indigo-50">
                  ...
                  <td class="text-xs text-gray-600">{{$item['address']}}</td>
                  <td class="text-xs text-gray-600"><a target="_blank" href="{{$item['map_link']}}">Click Here</a></td>
                  <td class="text-xs text-right font-medium text-gray-700">{{
                    number_format($townPop[$item['town_village_code']] * pow(1 + (str_replace('%','',$item['AEGR_pct'])
                    / 100), 15)) ?? 0}}</td>
                  <td class="text-xs text-center">
                    <a target="_blank"
                      href="{{ url('/') }}/city/{{ url_encoder($item['state_LGD_code'] . '-' . $item['district_LGD_code'] . '-' . $item['town_village_code']) }}/{{strtolower($item['town_village_name'])}}"
                      class="inline-block bg-indigo-100 text-indigo-700 px-2 py-1 rounded text-[10px] font-semibold hover:bg-indigo-200 transition-colors whitespace-nowrap">Click
                      <br> Here</a>
                  </td>
                </tr>
                @endforeach
      ...
      <div class="col-xl-12 col-lg-12 col-md-12">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden h-100 flex flex-col">
          <div class="bg-blue-600 text-white p-2 text-center font-bold text-sm tracking-wide uppercase">Village Webs
          </div>
          <div class="table-responsive flex-grow-1" style="max-height: 500px;">
            <table class="table table-hover table-striped table-sm m-0 border-0 align-middle">
              <thead class="sticky-top bg-light shadow-sm" style="z-index: 1;">
              ...
              <tbody>
                ...
                <tr class="transition-colors hover:bg-emerald-50">
                  ...
                  <td class="text-xs text-right font-medium text-gray-700">{{
                    number_format($villagePop[$item['town_village_code']] * pow(1 +
                    (str_replace('%','',$item['AEGR_pct']) / 100), 15)) ?? 0}}</td>
                  <td class="text-xs text-center">
                    <a target="_blank"
                      href="{{ url('/') }}/village/{{ url_encoder($item['state_LGD_code'] . '-' . $item['district_LGD_code'] . '-' . $item['town_village_code']) }}/{{strtolower($item['town_village_name'])}}"
                      class="inline-block bg-emerald-100 text-emerald-700 px-2 py-1 rounded text-[10px] font-semibold hover:bg-emerald-200 transition-colors whitespace-nowrap">Click
                      <br> Here</a>
                  </td>
                </tr>
                @endforeach
