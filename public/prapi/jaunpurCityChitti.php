...
{ 
   $sqlGeographyList=mysqli_query($dbconnect, "select replace(msubscribergeography.geographyCode,'r','') as areaId from msubscriberlist inner join msubscribergeography on msubscriberlist.subscriberId = msubscribergeography.subscriberId where msubscribergeography.geographyCode='r4' and msubscribergeography.subscriberId ='$subcriberId' ");
	...
	if($countt > 0)
	{
		...
		if($areaList == '0')
		{
			$sqlChittiList = mysqli_query($dbconnect, "select distinct chittiId from chittigeographymapping WHERE geographyId = '3'"); 
		}
		else
		{	
			$sqlChittiList = mysqli_query($dbconnect, "select distinct chittiId from chittigeographymapping WHERE geographyId = '3' and areaId IN ($areaList)"); 
	    }
...
// 	 die;
	...
	{
	   // die;
		$sqlGeographyList1=mysqli_query($dbconnect, "select * from msubscribergeography where subscriberId ='$subcriberId' and geographyId='3' and geographyCode='0' ");
		...
		else
		{
		  //  echo 'irfan';
		  //  die;
			$sqlChittiList = mysqli_query($dbconnect, "select distinct chittiId from chittigeographymapping WHERE geographyId = '3' and areaId=4");
	...
	{		
		if($offset < 1)
		{
			...
			//STR_TO_DATE(ch.dateOfApprove,  '%d-%m-%Y' )
		}
		else
		{ 
		...
		{			
			...
			{		
				...
				$areaId = $sqlChittiGeography['areaId'];
					...
					$sqlQueryArea= mysqli_fetch_assoc(mysqli_query($dbconnect, "select * from mregion where regionId =4")); 
					if($languageId =='1')
					{
				...
				$chitti["url"] = "http://prarang.in/jaunpur/posts/".$chitti["chittiId"].'/'.str_replace(' ','-',$obj['SubTitle']);//chittiDescription.php?chittiId=".$chitti["chittiId"];
