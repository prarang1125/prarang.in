...
$StartLimit = BlockSQLInjection($_REQUEST['StartLimit']);
...
if ($subcriberId == '') {
	...
	$sqlGeographyList = mysqli_query($dbconnect, "select replace(SG.geographyCode,'c','') as areaId from msubscribergeography SG inner join msubscriberlist SL on SL.subscriberId = SG.subscriberId where SG.geographyCode='$geographyCode' and SG.subscriberId =$subcriberId ");
	...
	if ($countt > 0) {
		...
		if ($areaList == '0') {
			...
			$sqlChittiList = mysqli_query($dbconnect, "select distinct chittiId from chittigeographymapping WHERE geographyId = '2' and areaId=$areaList");
		}
		...
		if (mysqli_num_rows($sqlGeographyList1) > 0) {
			...
			$areaId = str_replace('c', '', $geographyCode);
			$sqlChittiList = mysqli_query($dbconnect, "select distinct chittiId from chittigeographymapping WHERE geographyId = '2' and areaId=$areaId");
			...
			if ($countt1 > 0) {
				...
				// Fetch each row and store the IDs
				while ($displayChittiList = mysqli_fetch_array($sqlChittiList)) {
					$chittiIdList[$y] = $displayChittiList['chittiId'];
					$y++;
				}
				...
				$chittiList = ''; // Set to an empty string or a default value
			}
		}
	}
