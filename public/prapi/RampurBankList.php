...
{ 
    $sqlGeographyList=mysqli_query($dbconnect, "select chittiId from chittisavebank WHERE subscriberId='$subcriberId' and isSave=1 and GeographyCode='c3' ");
   
    $countt = mysqli_num_rows($sqlGeographyList);
	if($countt > 0)
	{
		$sqlChittiList = mysqli_query($dbconnect, "select distinct chittiId from chittisavebank WHERE subscriberId='$subcriberId' and isSave=1 and GeographyCode='c3'"); 
