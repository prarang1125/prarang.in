...
$query = "SELECT DISTINCT chitti.chittiId AS post_id, tagm.tagId AS tag_id, colorinfo.id AS emotion_id, colorinfo.name AS emotion, colorinfo.colorname AS emotion_color, facity.value AS for_about_city, chitti.totalViewerCount AS reach, chitti.fb_link_click, chitti.dateOfApprove AS post_date,DATE_FORMAT(STR_TO_DATE(chitti.dateOfApprove, '%d-%m-%Y'), '%Y-%m') AS month_year
FROM chitti
