...
class AIController extends Controller
{
    ...
    public function generateAIResponseApi(Request $request)
    {
        try {
            // Step 1: Validate input
            $request->validate([
                'prompt' => 'required|string',
                'model' => 'required|string|in:chatgpt,gemini,claude,grok',
            ]);
            ...
            // Step 3: Generate response based on model
            $response = null;
            $error = null;

            switch ($model) {
                case 'chatgpt':
                    $result = $this->aiService->generateGptResponse('gpt-4', [
                        'input' => $prompt,
                    ]);
                    break;
            ...
            if ($result['success']) {
                return response()->json([
                    ...
                    'raw' => $result['raw'],
                    'generated_at' => Carbon::now()->format('H:i:s d-m-Y'),
                ]);
            } else {
                return response()->json([
                    'success' => false,
